...
<div class="breadcrumb">
    ...
    <span class="breadcrumb-sep">/</span>
    <span class="breadcrumb-current">Edit Subscription</span>
</div>
...
<div class="form-card" style="margin: 0 auto;">
    <div class="form-card-header">
        ...
        <div>
            ...
            <p>Perbarui konfigurasi dan status langganan pelanggan</p>
        </div>
    </div>
    ...
    <form action="{{ route('subscriptions.update', $subscription) }}" method="POST">
        ...
        <div class="form-card-body">
            ...
            <div class="form-group">
                <label for="customer_id" class="form-label">Pelanggan <span style="color: var(--red);">*</span></label>
                <select id="customer_id" name="customer_id" class="form-control @error('customer_id') form-error @enderror" required>
                    ...
                    @foreach($customers as $c)
                        <option value="{{ $c->id }}" {{ old('customer_id', $subscription->customer_id) == $c->id ? 'selected' : '' }}>
                            {{ $c->name }} ({{ $c->customer_id }}) {{ !$c->status ? '[NONAKTIF]' : '' }}
                        </option>
                    @endforeach
                </select>
                @error('customer_id')
            ...
            {{-- Service --}}
            <div class="form-group">
                <label for="service_id" class="form-label">Layanan Digital <span style="color: var(--red);">*</span></label>
                <select id="service_id" name="service_id" class="form-control @error('service_id') form-error @enderror" required>
                    <option value="">-- Pilih Layanan --</option>
                    @foreach($services as $s)
                        <option value="{{ $s->id }}" {{ old('service_id', $subscription->service_id) == $s->id ? 'selected' : '' }}>
                            {{ $s->name }} — Rp {{ number_format($s->price, 0, ',', '.') }}/bln {{ !$s->status ? '[NONAKTIF]' : '' }}
                        </option>
                    @endforeach
                </select>
                @error('service_id')
            ...
            {{-- Status --}}
            <div class="form-group">
                <label for="status" class="form-label">Status Langganan</label>
                <select id="status" name="status" class="form-control @error('status') form-error @enderror">
                    @foreach($statuses as $st)
                        <option value="{{ $st }}" {{ old('status', $subscription->status) === $st ? 'selected' : '' }}>
                            {{ strtoupper($st) }}
                        </option>
                    @endforeach
                </select>
